# Black Jack

Welcome to my Black Jack game!

## How to Play
- **Run the game:** Double-click `BlackJackGame.exe` to start.
- **Controls:** Use your mouse to interact with the buttons on the screen (Hit, Stand, Double Down, Split).
- **Objective:** Beat the dealer without going over 21.

## Feedback
I am currently playtesting this version. If you run into any bugs, glitches, or crashes, please let me know! 

Enjoy the game!

BlackJack Game Credits

Card Art
Playing Table
Button Art
Created by Taylor Wolbach via Adobe Photoshop

Created using Godot 4.6.1

Coding
Created with Google Gemini

Audio
Muisc:
All Songs - Music by Alex Morgan from Pixabay

SFX:
Deck Shuffle (Shuffle Cards) - Sound Effect by freesound_community from Pixabay
Card Dealt Sound - Sound Effect by freesound_community from Pixabay
